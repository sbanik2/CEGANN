...
class gbf_expansion(nn.Module):
    ...
    def forward(self, data: torch.Tensor,bond = True) -> torch.Tensor:
        if bond:
            return torch.exp(-(data.unsqueeze(2) - self.filters)**2 /self.gamma**2)
        else:
            return torch.exp(-(data.unsqueeze(3) - self.filters)**2 /self.gamma**2)
    ...
    def forward(self, data: torch.Tensor) -> torch.Tensor:
        ...
        if self.l==1:
            ...
            for i in range(2,self.l):
                P = (1/(i+1))*((2*i+1)*data*retvars[i-1]-i*retvars[i-2])
                retvars.append(P)
...
class ConvAngle(nn.Module):
    ...
    def __init__(self,
        ...
        self.lin_angle = nn.Linear(self.angle_fea_len+2*self.edge_fea_len,
                                   self.angle_fea_len)
       
        #---------------Angle 3body attention------------------------
        
        
        self.attention_1 = nn.Linear(self.angle_fea_len+2*self.edge_fea_len,1)
...
class ConvEdge(nn.Module):
    ...
    def __init__(self,
        ...
        #-------------------Angle----------------------
        self.lin_edge =nn.Linear(2*self.edge_fea_len+self.angle_fea_len,self.edge_fea_len )
        
       
        #---------------edege attention------------------------
        
        self.attention_1 = nn.Linear(2*self.edge_fea_len+self.angle_fea_len,1)
        self.leakyrelu_1= nn.LeakyReLU(negative_slope=0.01)
...
class CEGAN(nn.Module):
    ...
    def __init__(self,
        ...
        self.EdgeConv = nn.ModuleList([ConvEdge(self.bond_fea_len ,
                                             self.angle_fea_len
                                            ) for _ in range(n_conv_edge)])
        self.AngConv = nn.ModuleList([ConvAngle(self.bond_fea_len ,
                                             self.angle_fea_len
    ...
    def forward(self,data):
       ...
       # print(edge_fea.shape)
        ...
        angle_fea = torch.sum(self.conv_to_fc_softplus(torch.sum(self.conv_to_fc_softplus(angle_fea),dim=2)),dim=1)
    ...
    def pool(self, atom_fea, crys_idx):
       ...
       # print(crystal_atom_idx)
        ...
        summed_fea = [torch.mean(atom_fea[idx_map[0]:idx_map[1],:], dim=0, keepdim=True)
