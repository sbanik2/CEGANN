...
def train(graphs,settings,model,output_dir=os.getcwd()+'model',screen_log ="log.model"):
    ...
    train_loader,val_loader,test_loader =  get_train_val_test_loader(graphs, 
    ...
    if settings.optimizer == "sgd":   
        optimizer = optim.SGD(model.parameters(), lr=settings.learning_rate, momentum=settings.momentum)
    ...
    def Checkpoint(epoch,is_best,path = os.getcwd(),filename ='checkpoint.pt'):
        ...
        if is_best:
            shutil.copyfile("{}/{}_{}".format(path,filename,epoch), "{}/model_best.pt".format(path))
    ...
    if settings.resume:
        ...
        if os.path.isfile("{}/checkpoint.pt".format(output_dir)):
            print("=> loading checkpoint '{}'".format("{}/checkpoint.pt".format(output_dir)))
    ...
    def log_training_results(engine):
        ...
        if val_accuracy> best_val_accuracy: 
            ...
            print("Saving the data for best accuracy of {} obtained at epoch {}".format(best_val_accuracy,epoch))
            Checkpoint(epoch,is_best=True,path=output_dir) # add checkpoint function here

        if len(test_loader)!=0:
            pbar.log_message("Avg_Train_loss:{:4f},  Accuracy_val: {:4f},  Accuracy_test: {:4f}".format(avg_batch_loss,val_accuracy,test_accuracy))
            
            with open(screen_log,"a") as outfile:
                outfile.write("{:4f},{:4f},{:4f}\n".format(avg_batch_loss,val_accuracy,test_accuracy))
                  
        else:
            pbar.log_message("Avg_Train_loss:{:4f},  Accuracy_val: {:4f}".format(avg_batch_loss,val_accuracy))
            with open(screen_log,"a") as outfile:
